 ...
 *
 * Незакрытые заказы отдаются без ограничения по дате (заказ, зависший со вчера,
 * никуда не делся), закрытые — только за сегодня: вкладки «Geliefert» и
 * «Storniert» подводят итог смены, а не истории.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [activeRows, finishedRows, settings] = await Promise.all([
      Order.find({ status: { $in: [...POS_ACTIVE_ORDER_STATUSES] } })
        .sort({ createdAt: 1 })
        .lean(),
      Order.find({
