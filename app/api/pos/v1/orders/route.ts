...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      jobs,
      /**
       * «Печатать нечего», а НЕ «выключена настройка».
       *
       * Прибор понимает этот флаг буквально: увидев его, служба печати пишет
       * «Печать выключена в админке» и не печатает НИЧЕГО — даже задание, которое
       * человек только что запросил кнопкой. Поэтому при выключенной автопечати
       * флаг поднимается лишь тогда, когда отдавать действительно нечего.
       *
       * Правится здесь, а не на приборе, намеренно: обновление apk требует похода
       * на кухню, а печать нужна сегодня. Прибору достаточно знать, печатать ли
       * то, что ему прислали, — решает всё равно сервер.
       */
      paused: !settings.enabled && jobs.length === 0,
