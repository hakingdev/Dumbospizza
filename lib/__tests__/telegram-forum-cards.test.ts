...
describe('renderCardKeyboard', () => {
  ...
  it('кнопки только релевантные текущему статусу', () => {
    ...
    // «Вернуть в готовку» и здесь: в «В пути» заказ попадает одним касанием
    // с терминала, и ошибочный перевод должен быть откатываемым (#260820002).
    expect(texts('on_the_way')).toEqual([
      '✅ Доставлен',
  ...
  it('возврат в готовку из «В пути» идёт существующим status_preparing', () => {
    const back = renderCardKeyboard({ ...base, status: 'on_the_way' })
      .inline_keyboard.flat()
      .find((b) => b.text === '↩️ Вернуть в готовку');
    expect(back?.callback_data).toBe('status_preparing_260620001');
  });
